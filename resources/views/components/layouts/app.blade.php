                ...
                @if ($user = auth()->user())
                    ...
                    @if (auth()->user()->role_id == 1)
                        ...
                        <x-menu-sub title="Data Master" icon="fas.database">
                            <x-menu-item title="Data Kriteria" icon="fas.clipboard-list" link="/kriteria" />
                            <x-menu-item title="Data Cafe" icon="fas.coffee" link="/cafe" />
                            <x-menu-item title="Data Alternatif" icon="fas.tasks" link="/alternatif" />
                        </x-menu-sub>
                    @endif

                    @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 2)
                        <x-menu-sub title="Hasil Penelitian" icon="fas.chart-bar"> {{-- ikon chart-bar lebih umum untuk
                            hasil/analisis --}}
                            <x-menu-item title="Rangking" icon="fas.trophy" link="/rangking" /> {{-- ikon trophy untuk
                            ranking/penghargaan --}}
                            <x-menu-item title="Rekomendasi" icon="fas.lightbulb" link="/rekomendasi" />
                            {{-- ikon lightbulb
                            untuk ide/rekomendasi --}}
                        </x-menu-sub>
                    @endif
                @endif
            </x-menu>
